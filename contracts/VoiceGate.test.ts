...
import { VoiceGate, type VoiceGateDeps } from "./VoiceGate";
...
describe("VoiceGate", () => {
  ...
  it("allows voice curl from main session (terminal detected)", () => {
    ...
    const result = VoiceGate.execute(makeInput('curl localhost:8888'), deps) as any;
  ...
  it("blocks voice curl from subagent (no terminal)", () => {
    const deps = makeDeps({
      ...
      getItermSessionId: () => undefined,
      existsSync: () => false, // no kitty sessions dir
    });
    ...
    const result = VoiceGate.execute(makeInput('curl localhost:8888'), deps2) as any;
    expect(result.ok).toBe(true);
