...
- The template file cannot be read (returns `ok({ continue: true })`, fails open)
...
3. Verifies the template file exists; returns `ok({ continue: true })` if missing
...
8. Returns `ok({ hookSpecificOutput: { hookEventName: "PreToolUse", updatedInput: { prompt } } })` to modify the Agent tool's input
