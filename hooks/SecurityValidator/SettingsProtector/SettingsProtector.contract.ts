/**
 * SettingsProtector Contract — Guard settings.json from uncontrolled mutation.
 *
 ...
 * (SettingsProtectorPost) compares the snapshot to current state after the
...
export interface SettingsProtectorDeps {
  homedir: () => string;
...
const defaultDeps: SettingsProtectorDeps = {
...
export const SettingsProtector: SyncHookContract<
  ToolHookInput,
  ContinueOutput | AskOutput,
  SettingsProtectorDeps
> = {
  name: "SettingsProtector",
  event: "PreToolUse",
  ...
  execute(
    ...
    if (input.tool_name === "Bash") {
      ...
      // PostToolUse (SettingsProtectorPost) will compare after.
    ...
    deps.stderr(`[SettingsProtector] ${input.tool_name} targets settings file: ${filePath}`);
    logSettingsAudit({
      ts: new Date().toISOString(),
      session_id: input.session_id,
