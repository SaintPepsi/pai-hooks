...
| `tool-input.ts`               | Type guards (`isWriteInput`, `isEditInput`, `isBashInput`, `isReadInput`, `isGlobInput`, `isGrepInput`, `isSkillInput`, `isAgentInput`, `isTaskCreateInput`, `isTaskUpdateInput`) that narrow `ToolHookInput` to per-tool shapes, plus canonical extractors (`getFilePath`, `getWriteContent`, `getCommand`) |
